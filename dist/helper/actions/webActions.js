...
const isPatternEnabled = String(global_1.vars.getConfigValue("patternIQ.enable")).toLowerCase().trim() ===
    "true";
/**
...
async function waitForTextAtLocation(page, field, expectedText, options) {
    ...
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout")) || 10000, partialMatch = false, ignoreCase = true, screenshot = false, screenshotText = "", screenshotFullPage = true, pattern, } = options_json;
    if (!page)
...
async function waitForHeader(page, header, headerText, options) {
    ...
    async function doWaitForHeader() {
        ...
        const target = typeof header === "string"
            ? await (0, global_1.webLocResolver)("header", header, page, pattern, actionTimeout)
            : header;
        const start = Date.now();
        let found = false;
        let lastActualText = "";
...
async function fill(page, field, value, options) {
    ...
    if (!page)
        throw new Error("Page not initialized");
    const { iframe = "", actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout")) || 30000, // Default timeout
    ...
    async function doFill() {
        const target = typeof field === "string"
            ? await (0, global_1.webLocResolver)("input", field, page, pattern, smartIQ_refreshLoc)
            : field;
        if (iframe) {
...
async function clickButton(page, field, options) {
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout") || 10000), pattern, isDoubleClick = false, screenshot = false, screenshotText = "", screenshotFullPage = true, screenshotBefore = false, } = options_json || {};
    if ((0, runner_1.isPlaywrightRunner)()) {
...
async function clickLink(page, field, options) {
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout") || 10000), // Default timeout
...
async function clickRadioButton(page, field, options) {
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout")) || 30000, // Default timeout
...
async function selectDropdown(page, field, value, options) {
    ...
    async function doSelectDropdown() {
        ...
        const target = typeof field === "string"
            ? await (0, global_1.webLocResolver)("dropdown", field, page, pattern, actionTimeout, smartIQ_refreshLoc)
            : field;
        // Determine tag
...
async function verifyHeaderText(page, expectedText, options) {
    ...
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout") || 10000), navigationTimeout = Number(global_1.vars.getConfigValue("testExecution.navigationTimeout") || 20000), partialMatch = false, ignoreCase = false, assert = true, screenshot = true, screenshotText = "", screenshotFullPage = true, } = options_json;
    if ((0, runner_1.isPlaywrightRunner)()) {
...
async function verifyInputFieldPresent(page, field, options) {
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout")) || 30000, pattern, assert = true, screenshot = true, screenshotText = "", screenshotFullPage = true, } = options_json;
    if (!page)
        throw new Error("Page not initialized");
    await waitForPageToLoad(page, actionTimeout);
    const target = typeof field === "string"
...
async function verifyTabField(page, field, options) {
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout")) || 30000, pattern, assert = true, isPresent = true, isEnabled = false, isSelected = false, isNotSelected = false, screenshot = true, screenshotText = "", screenshotFullPage = true, } = options_json;
    if ((0, runner_1.isPlaywrightRunner)()) {
...
async function waitForInputState(page, field, state, options) {
    ...
    async function doWaitForInputState() {
        if (!page)
            throw new Error("Page not initialized");
        const target = typeof field === "string"
            ? await (0, global_1.webLocResolver)("input", field, page, pattern, actionTimeout, smartIQ_refreshLoc)
            : field;
        try {
            if (state === "enabled") {
...
async function waitForUrl(page, url, options) {
    const options_json = typeof options === "string" ? (0, vars_1.parseLooseJson)(options) : options || {};
    const { actionTimeout = Number(global_1.vars.getConfigValue("testExecution.actionTimeout")) || 30000, match = "contains", screenshot = false, screenshotText, screenshotFullPage = true, } = options_json;
    if ((0, runner_1.isPlaywrightRunner)()) {
